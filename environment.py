...
from movement import Direction, Square, handle_movement, SnakeData
import numpy as np
import torch
import pickle
...
class SnakeEnvironment:
    def __init__(self, config, save=False): # config must be a dictionary containing 'GRID_WIDTH' and 'GRID_HEIGHT' entries
        ...
        head1_dir = Direction.RIGHT  # FIXME: why does this break if not initialized here?
    ...
    def step(self, input1, input2, update_state=True):
        ...
        head_pos1 = snake1.path[0]
        if head_pos1[0] > 5 and head_pos1[0] < 15 and head_pos1[1] > 5 and head_pos1[1] < 15:
            reward1 += 10
