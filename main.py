...
def main():
    ...
    # ai_vs_ai('final_blind_model.pt', 7, 256, 3)

    config = {
        'GRID_WIDTH': 20,
        'GRID_HEIGHT': 20
    }
    # train(config, 'final_blind_model.pt', 7, 256, 3, best_rewards=0, episodes=500)

    test(config, 'final_blind_model.pt', 7, 256, 3, episodes=100)
...
def ai_vs_ai(path, n_in, n_hidden, n_out, ticks_per_s=30):
    agent1 = Agent(path, n_in, n_hidden, n_out)
    agent2 = Agent(path, n_in, n_hidden, n_out)
